...
#ifndef IN_MEMORY_SPATIAL_DB_MXCIFQUADNODEV1_H
#define IN_MEMORY_SPATIAL_DB_MXCIFQUADNODEV1_H

#include "quadTreeLib.h"
#include "qBoundingBox.h"

class mxcifQuadNodeV1
{
private:
...
public:
    ...
    mxcifQuadNodeV1 *mxqnNW;
    mxcifQuadNodeV1 *mxqnNE;
    mxcifQuadNodeV1 *mxqnSW;
    mxcifQuadNodeV1 *mxqnSE;




    mxcifQuadNodeV1() {}
    mxcifQuadNodeV1(qBoundingBox QBB)
    {
        this->height = 0;
        this->leafCapacity = 4;
        (*this).qbb = QBB;
    ...
    mxcifQuadNodeV1(qBoundingBox QBB, int leafCapacity)
    {
        this->height = 0;
    ...
    mxcifQuadNodeV1(qBoundingBox QBB, int leafCapacity,int height)
    {
        this->height = height;
        this->leafCapacity = leafCapacity;
        (*this).qbb = QBB;
    ...
    mxcifQuadNodeV1(int originX, int originY, float width, float height)
    {
        this->height = 0;
        this->leafCapacity = 4;
...
//        if (w < 1 || h < 1) return ;
        ...
        this->mxqnNW = new mxcifQuadNodeV1(*qbbNW,this->leafCapacity,cur_height);


        qPoint *pNE = new qPoint(current_x,current_y);
        qBoundingBox *qbbNE = new qBoundingBox( pNE, w, h);
        this->mxqnNE = new mxcifQuadNodeV1(*qbbNE,this->leafCapacity,cur_height);


        qPoint *pSW = new qPoint(current_x-w,current_y-h);
        qBoundingBox *qbbSW =new qBoundingBox( pSW, w, h);
        this->mxqnSW = new mxcifQuadNodeV1(*qbbSW,this->leafCapacity,cur_height);


        qPoint *pSE = new qPoint(current_x,current_y-h);
        qBoundingBox *qbbSE =new qBoundingBox( pSE, w, h);
        this->mxqnSE = new mxcifQuadNodeV1(*qbbSE,this->leafCapacity,cur_height);
    ...
    bool subdivide(qBoundingBox *box)
    {
        ...
        qPoint *pNW = new qPoint(current_x-w,current_y);
        qBoundingBox *qbbNW = new qBoundingBox( pNW, w, h);
        this->mxqnNW = new mxcifQuadNodeV1(*qbbNW);


        qPoint *pNE = new qPoint(current_x,current_y);
        qBoundingBox *qbbNE = new qBoundingBox( pNE, w, h);
        this->mxqnNE = new mxcifQuadNodeV1(*qbbNE);


        qPoint *pSW = new qPoint(current_x-w,current_y-h);
        qBoundingBox *qbbSW =new qBoundingBox( pSW, w, h);
        this->mxqnSW = new mxcifQuadNodeV1(*qbbSW);


        qPoint *pSE = new qPoint(current_x,current_y-h);
        qBoundingBox *qbbSE =new qBoundingBox( pSE, w, h);
        this->mxqnSE = new mxcifQuadNodeV1(*qbbSE);
...
//        if(!overFlowRectangles.empty()){overFlowRectangles.clear();}
    ...
    void print(string prefix = "", bool emptyLine = true)
    {
        ...
        cout<<prefix<< "mxcifQuadNodeV1 | height : "<<this->height<<endl;
        cout<<prefix<< "mxcifQuadNodeV1 | ";
        this->qbb.print("\t\t",true);
        cout<<prefix<< "mxcifQuadNodeV1 | rectangles Size = "<<this->rectangles.size()<<" | overFlowRectangles Size = "<<this->overFlowRectangles.size();
...
#endif //IN_MEMORY_SPATIAL_DB_MXCIFQUADNODEV1_H
