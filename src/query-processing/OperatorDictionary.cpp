...
bool OperatorDictionary::applyFilterBy (vector<string> filterDetails, Point inputPoint) {
	...
	if (filterDetails[1] == "distance") {
		...
		if (filterDetails.size()==8) {
			metricVal = PointOperations::getDistance(inputPoint,Rectangle(stod(filterDetails[4]),stod(filterDetails[5]),stod(filterDetails[6]),stod(filterDetails[7])));
		}
		else {
			metricVal = PointOperations::getDistance(inputPoint,Point(stod(filterDetails[4]),stod(filterDetails[5])));
		}
...
bool OperatorDictionary::applyFilterBy (vector<string> filterDetails, Rectangle inputRect) {
	...
	if (filterDetails[1] == "distance") {
		double metricVal;
		if (filterDetails.size()==8) {
			metricVal = RectangleOperations::getDistance(inputRect,Rectangle(stod(filterDetails[4]),stod(filterDetails[5]),stod(filterDetails[6]),stod(filterDetails[7])));
			//cout << metricVal << " - ";
		}
		else {
